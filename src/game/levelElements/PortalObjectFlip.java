 ...
 */
public class PortalObjectFlip extends GameObject implements Collidable {
    ...
    public PortalObjectFlip(float x, float y) {
        position.x = x;
        position.y = y;

        for (int i = 0; i < 8; i++) {
            assets[i] = AssetHandler.getAsset("portalflip-"+(i+1)+".png");
        }
    }
